<?php
	...
	class solicitantes_manutencao {
		...
		function solicitantes_manutencao() {
		...
		function listar_solicitantes() {			
					...
					$sql = "select * from tbl_solicitantes where und_nome like '%".$filtrar_por."%' order by ".$ordenacao;
		...
		function excluir_solicitantes() {
			
			// Armazena a query sql na variável "$sql". $_REQUEST['codigo'] pega o parâmetro codigo que é passado na url.
			$sql = "delete from tbl_solicitantes where und_codigo in (".$_REQUEST['codigo'].")";
		...
		// Excluir as solicitantes.
		function excluir_todos_solicitantes() {
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST')
			{
				...
				if(!@$_POST['excluir']){
					...
					// Armazena a query sql na variável "$sql". $_REQUEST['codigo'] pega o parâmetro codigo que é passado na url.
					$sql = "delete from tbl_solicitantes where und_codigo in (".implode(',', $arr).")";
		...
		// Incluir as solicitantes.
		function gravar_incluir_solicitantes() {
			
			if($_POST['und_nome'] <> "" || $_POST['und_nome'] <> null){
				...
				$sql = "insert into tbl_solicitantes (und_nome) values (upper('".$_POST['und_nome']."'))";
				...
				echo "<h4 class='alerta'>Campo nome do solicitante com preenchimento obrigat&oacute;rio.</h4>";

			}
				
		}
		
		// Alterar as solicitantes.
		function listar_alterar_solicitantes() {
													
					// Armazena a query sql na variável "$sql".
					$sql = "select * from tbl_solicitantes where und_codigo= ".$_REQUEST['codigo'];
		...
		function gravar_alterar_solicitantes() {
				...
				if($_POST['und_nome'] <> "" || $_POST['und_nome'] <> null){					
					...
					// Se as informações estão sendo passadas pelo formulário é mais seguro sempre usar o "POST".
					// Armazena a query sql na variável "$sql". 
					$sql = "update tbl_solicitantes set und_nome= upper('".$_POST['und_nome']."') where und_codigo= ".$_POST['codigo'];
		...
		function total_registros_solicitantes() {
			
			// Armazena a query sql na variável "$sql".
			$sql = "select count(*) as TOTAL from tbl_solicitantes";
